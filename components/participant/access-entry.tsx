...
import { validateAccessCode } from "@/lib/ist-logic";

export function AccessEntry() {
  const router = useRouter();
  const [code, setCode] = useState("IST-7K4M9Q2D");
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.ok && result.nextRoute) {
      setFeedbackTone("info");
      router.push(result.nextRoute);
      return;
    }

    setFeedbackTone(result.status === "expired" ? "warning" : "danger");
  }

  return (
    <section className="h-full min-w-0 w-full overflow-hidden rounded-2xl border border-[var(--border-default)] bg-[var(--surface-panel)] shadow-[var(--shadow-elevated)]">
      <div className="grid min-w-0 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="lms-grid-pattern relative min-w-0 overflow-hidden bg-[#4B0D61] px-6 py-10 text-white sm:px-10 sm:py-14">
        ...
        <div className="min-w-0 p-6 sm:p-10 lg:p-12">
          ...
          <form className="mt-7 min-w-0 space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="inline-flex h-12 w-full items-center justify-center rounded-xl bg-[#4B0D61] px-5 text-sm font-semibold text-white hover:bg-[#350444]"
            >
              Verifikasi dan buka tutorial
            </button>
            {feedback ? (
              <p
                role="alert"
          ...
          <div className="mt-7 border-t border-[var(--border-subtle)] pt-5">
            ...
            <div className="mt-4 flex flex-wrap gap-4 text-sm font-semibold">
              <Link className="text-[var(--accent-primary)] hover:underline" href="/hr">
                Portal HR
              </Link>
              <Link className="text-[var(--text-secondary)] hover:underline" href="/admin/users">
                Super Admin
              </Link>
            </div>
          </div>
