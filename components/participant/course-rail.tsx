...
export function CourseRail({ currentCode }: { currentCode: SubtestCode }) {
  ...
  return (
    <aside className="hidden h-full rounded-xl border border-border bg-card p-5 xl:sticky xl:top-24 xl:block self-start">
      <h2 className="mt-2 text-lg font-bold tracking-[-0.03em] text-foreground">IST Assessment</h2>
        ...
        {subtests.map((subtest, index) => {
          ...
          return (
            <li
              ...
              className={`flex min-w-0 items-center gap-3 rounded-xl px-3 py-2.5 ${isCurrent ? "bg-accent" : ""}`}
            >
              <span
                className={`grid size-8 shrink-0 place-items-center rounded-lg text-xs font-bold ${isComplete ? "bg-primary text-white" : isCurrent ? "bg-[var(--primary)] text-foreground" : "bg-muted text-muted-foreground"}`}
              >
                {isComplete ? "✓" : index + 1}
              </span>
